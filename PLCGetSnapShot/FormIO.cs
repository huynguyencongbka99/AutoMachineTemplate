...
namespace PLCGetSnapShot
{
    public partial class frmIO : Form
    {
        ...
        Button[] _btnXjjY;
        Button[] _btnXjjM;
        public frmIO(IPlcService _PLCservice, PLCServiceXinjie _plcXinjie)
        {
            InitializeComponent();
            this.plcService = _PLCservice;
        ...
        {
            var snapshot = plcService.GetSnapshot();
            var snapshotXinjie = plcXinjie.GetSnapshot(); 
            UpdateButtonsX(snapshot.X);
            UpdateButtonsY(snapshot.Y);
            UpdateButtonsM(snapshot.M);
